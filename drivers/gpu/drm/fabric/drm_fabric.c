...
/* ID 0 is the unassigned sentinel, so allocated IDs start at 1. */
 ...
 * unassigned endpoints (@fabric == NULL) do not participate.
 */
 ...
 *
 * A NULL @fabric registers an unassigned endpoint, which a later ENDPOINT_SET
...
void drm_fabric_endpoint_unregister(struct drm_fabric_endpoint *ep)
{
	...
	/* An unassigned endpoint holds no fabric reference. */
	if (ep->fabric)
			    ...
			    const struct drm_fabric_endpoint_change *req)
{
	...
	scoped_guard(mutex, &drm_fabric_lock) {
		...
		/* Attach: endpoint must be unassigned. */
		if (ep->fabric)
			return -EBUSY;
